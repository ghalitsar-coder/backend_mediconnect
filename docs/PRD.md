...
| Aspek | Target | Detail |
|-------|--------|--------|
| **Response Time** | P99 < 500ms · P50 < 200ms | Cache hit rate target ≥ 85% |
| **Skalabilitas** | HPA: min 2 · max 10 pods | Trigger CPU > 70% |
| **Ketersediaan** | SLA 99.5% uptime/bulan | Liveness + Readiness probe |
| **Keamanan** | JWT RS256 + AES-256 + TLS 1.3 | OWASP Top 10 compliance |
| **Keandalan Pesan** | 0 message loss | Queue durable + DLQ (3 retry) |
| **Observabilitas** | Full stack | Prometheus + Grafana + OpenTelemetry |
...
| Step | Nama | Deskripsi |
|------|------|-----------|
| S1 | **Checkout & lint** | git checkout → golangci-lint → ESLint |
| S2 | **Unit test** | `go test ./...` coverage ≥ 80% |
| S3 | **Integration test** | Spin up DB + Redis + RabbitMQ via Docker Compose |
| S4 | **Build & push Docker image** | Multi-stage build → image scan (Trivy) → push ke registry |
| S5 | **Deploy staging** | `kubectl set image` → `kubectl rollout status` |
| S6 | **Smoke test & approval** | Smoke test + manual gate sebelum production |
| S7 | **Deploy production** | Rolling update + rollback otomatis jika health check gagal |
...
|----------|--------|----------|
| 🔴 Tinggi | **Kebocoran data rekam medis** | Enkripsi AES-256, JWT scope-based, audit log immutable, pentest |
| 🔴 Tinggi | **API Dukcapil tidak tersedia** | Circuit breaker. Fallback: validasi NIK format-only |
| 🟡 Sedang | **Redis down** | Redis Sentinel/Cluster. Fallback: query PostgreSQL |
| 🟡 Sedang | **Lonjakan traffic** | HPA + load test k6 + rate limiting |
| 🔵 Rendah | **WhatsApp API rate limit** | RabbitMQ menyerap burst. Email sebagai fallback |
...
| MTTR | **< 30 menit** | Rollback otomatis K8s. |

---

## 11 · Open Questions

> ❓ **Integrasi Dukcapil:** Apakah sudah ada MOU dengan Disdukcapil untuk akses API validasi NIK?

> ❓ **Enkripsi rekam medis:** Apakah kunci enkripsi dikelola secara terpusat (KMS) atau per-tenant?

> ❓ **Multi-tenancy:** Apakah setiap Puskesmas punya namespace K8s sendiri, atau satu cluster shared dengan RBAC?

> ❓ **Regulasi data kesehatan:** Perlu konfirmasi apakah sistem harus memenuhi standar PERMENKES terkait rekam medis elektronik.

> ❓ **SLA WhatsApp:** Apakah ada fallback SMS jika WhatsApp Business API tidak tersedia?
